...
class GameStateManager:
    ...
    def transition_to_warmup(self) -> None:
        ...
        self.send_command("vstr d1")
        self.send_command(f"set g_warmup {settings.warmup_timelimit}")
        self.send_command(
            f"say Starting warmup round: {settings.warmup_timelimit} minute(s)!"
        )
        self.send_command("say Connecting to OBS instances...")
    ...
    def transition_to_running(self) -> None:
        ...
        if self.current_state != GameState.WARMUP:
            self.logger.warning(
                f"Invalid transition to RUNNING from {self.current_state}"
            )
            return
        ...
        self.send_command(
            f"say Experiment starting! Round {self.round_count + 1}/{self.max_rounds}"
        )
    ...
    def handle_match_end(self) -> dict:
        ...
        if self.current_state == GameState.RUNNING:
            if self.round_count >= self.max_rounds:
                ...
                self.send_command(
                    f"say Round {self.round_count}/{self.max_rounds} starting..."
                )
    ...
    def send_waiting_message(self, current_players: int, threshold: int) -> None:
        ...
        if self.current_state == GameState.WAITING:
            self.send_command(
                f"say WAITING ROOM: {current_players}/{threshold} players connected"
            )
    ...
    def check_obs_ready(self, obs_manager, client_manager) -> bool:
        ...
        all_connected = all(obs_manager.is_client_connected(ip) for ip in human_ips)
        ...
        if all_connected:
            ...
            self.logger.info(
                f"OBS connections: {connected_count}/{len(human_ips)} ready"
            )
