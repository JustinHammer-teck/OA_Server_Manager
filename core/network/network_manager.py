...
class NetworkManager:
    ...
    BOT_NAMES = [
        "Angelyss", "Arachna", "Major", "Sarge",
        "Skelebot", "Merman", "Beret", "Kyonshi",
    ]
    ...
    def add_client(self, client_id: int, ip: Optional[str] = None,
                   latency: Optional[int] = None, name: Optional[str] = None,
        ...
        if not is_bot and ip:
            if ip not in self.ip_latency_map:
                ...
                self.logger.info(f"Added HUMAN client {client_id} with IP {ip}, latency {latency}ms")
            else:
                self.logger.debug(f"Client IP {ip} already exists, updating client_id mapping")
                self.client_ip_map[client_id] = ip
            self.human_count = len([cid for cid, ctype in self.client_type_map.items() if ctype == "HUMAN"])
        elif is_bot:
            self.logger.info(f"Added BOT client {client_id} with name {name}")
            self.bot_count = len([cid for cid, ctype in self.client_type_map.items() if ctype == "BOT"])
    ...
    def remove_client(self, client_id: int) -> None:
        ...
        if client_id in self.client_ip_map:
            ...
            if ip not in self.client_ip_map.values():
                ...
                self.logger.info(f"Removed {client_type} client {client_id} with IP {ip}")
            else:
    ...
    def get_client_info_table(self) -> List[List[Any]]:
        ...
        for client_id, client_type in self.client_type_map.items():
            ...
            if client_type == "HUMAN":
                ...
                row.append(ip)
                row.append("HUMAN")
                row.append(f"{self.ip_latency_map.get(ip, 0)}ms" if ip != "N/A" else "N/A")
    ...
    def apply_latency_rules(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Applied latency rules to {len(self.ip_latency_map)} clients on interface {self.interface}")

            if self.send_command:
                self.send_command(f"say Latency rules applied to {len(self.ip_latency_map)} clients")
