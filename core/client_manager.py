...
class ClientManager:
    ...
    def add_client(self, client_id: int, ip: Optional[str] = None, 
        ...
        # Determine if it's a bot based on name or explicit flag
        ...
        # Handle IP and latency for non-bot clients
        if not is_bot and ip:
            if ip not in self.ip_latency_map:
                ...
                self.client_ip_map[client_id] = ip
            self.human_count = len([cid for cid, ctype in self.client_type_map.items() 
                                     if ctype == "HUMAN"])
        elif is_bot:
            self.logger.info(f"Added BOT client {client_id} with name {name}")
            self.bot_count = len([cid for cid, ctype in self.client_type_map.items() 
                                   if ctype == "BOT"])
    ...
    def remove_client(self, client_id: int) -> None:
        ...
        # Check client type before removal
        ...
        # Remove from type and name maps
