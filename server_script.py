...
parser = argparse.ArgumentParser("Start openarena server, wait for clients to connect, start experiment sequence, and upload results")
...
parser.add_argument('--bots', default=bot_count, type=ranged_type(int, 0, 10), 
                   help='Number of bots to add (0-10 inclusive)')
parser.add_argument('--difficulty', default=bot_difficulty, type=ranged_type(int, 1, 5),
...
def apply_latency_rules(ip_latency_map, interface):
    ...
    os.system("/usr/bin/sudo nft add chain ip netem output '{ type filter hook output priority 0; }' || true")

    # Set up HTB qdisc
    print("Setting up HTB qdisc...")
    os.system(f"/usr/bin/sudo /sbin/tc qdisc add dev {interface} root handle 1: htb default 1")
    ...
    for i, (ip, latency) in enumerate(ip_latency_map.items(), start=1):
        ...
        os.system(f"/usr/bin/sudo /sbin/tc class add dev {interface} parent 1: classid {class_id} htb rate 1000mbit")

        # Apply netem to this class
        os.system(f"/usr/bin/sudo /sbin/tc qdisc add dev {interface} parent {class_id} handle {i+10}: netem delay {latency}ms")

        # Use tc filter to assign marked packets to the correct class
        os.system(f"/usr/bin/sudo /sbin/tc filter add dev {interface} protocol ip parent 1: prio 1 handle {mark_id} fw classid {class_id}")

        # Use nftables to mark packets based on destination IP
        os.system(f"/usr/bin/sudo nft add rule ip netem output ip daddr {ip} meta mark set {mark_id}")
...
def signal_handler(sig, frame):
    ...
    os.system(f"/usr/bin/sudo /sbin/tc qdisc replace dev {args.interface} root pfifo_fast")
    sys.exit(0)
...
try:
    ...
    print("Error: unable to open output file \"%s\" for writing: %s" % (filename, os.strerror(errno)), file=sys.stderr)
    sys.exit(-2)

print("Reading server output; conditions set to nominal", file=sys.stderr)

# Open pipe to ioquake3:
server = Popen(['oa_ded', "+set", 'dedicated', '1', '+set', 'net_port', '27960',
               '+set', 'com_legacyprotocol', '71', '+set', 'com_protocol', '71', 
               '+set', 'sv_master1', 'dpmaster.deathmask.net', '+set', 'cl_motd', '0', 
               '+set', 'com_homepath', 'server.oa', '+exec', 'andrew_server.cfg'], 
               stdout=PIPE, stdin=PIPE, stderr=PIPE, universal_newlines=False)
...
while (True):
    ...
    m = re.match("^Client ([0-9]+) connecting with ([0-9]+) challenge ping$", msgFromServer)

    # If a new player has connected...
    if m:
        print("Client %i has connected with %i challenge ping." % (int(m.group(1)), int(m.group(2))), file=sys.stderr)
        ...
        if STATE == state.WAITING:
            if nplayers >= nplayers_threshold:
                ...
                server.stdin.write(f"say Starting warmup round: {warmup_timelimit} minutes!\r\n".encode())
                server.stdin.flush()

                STATE = state.WARMUP
                print("State changed from WAITING to WARMUP")
            else:
                server.stdin.write(f"say WAITING ROOM: {nplayers}/{nplayers_threshold} players connected\r\n".encode())
                server.stdin.flush()
    ...
    if nplayers >= nplayers_threshold and m:
        if STATE == state.RUNNING:
            ...
            if i == 0:
                print("Warmup round complete: applying initial network conditions.", file=sys.stderr)
                print("%f: Warmup round complete: applying initial network conditions." % t, file=log)
            else:
                print("Round %i complete: changing network conditions." % i, file=sys.stderr)
                print("%f: Round %i complete: changing network conditions." % (t, i), file=log)
