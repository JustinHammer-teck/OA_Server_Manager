...
server = Server()
async_loop = None
interface = "enp1s0"  # Default interface, will be updated from args
...
Examples:
  ...
  python main.py --no-bots
    ...
    bot_group.add_argument(
        '--add-bots',
        type=int,
        help='Add additional bots to default count'
    )
    bot_group.add_argument(
        '--no-bots',
        action='store_true',
        help='Disable bots completely'
    )
    ...
    server_group.add_argument(
        ...
        type=str,
        default='enp1s0',
        help='Network interface for latency control (default: enp1s0)'
    )
...
def configure_bots_from_args(args):
    ...
    bot_config = {
        ...
        'names': settings.bot_names if settings.bot_names[0] else ["Sarge", "Bones", "Slash", "Grunt", "Major", "Ranger"]
    }
    ...
    # Ensure we have bots enabled if count > 0
...
def main():
    ...
    # Apply command line arguments to bot configuration
    ...
    try:
        # Pass bot and interface configuration to server
